...
#
# PlayerAshe.gd (custom version)
#
# Tato verze explicitně přepisuje exportovanou proměnnou `bullet_scene` zděděnou z
# `CharacterBase.gd` tak, aby Ashe používala novou šípovou střelu definovanou ve
# scéně `Scenes/bullet-ashe.tscn`. Pokud budete chtít použít jinou střelu, můžete
# cestu jednoduše upravit zde nebo v editoru.

@export var bullet_scene: PackedScene = preload("res://Scenes/bullet-ashe.tscn")
...
func _cast_skill_q() -> void:
    ...
    for i in range(count):
        ...
        # pokud má bullet nějaké extra metody (speed, pierce, split), můžeme je sem časem přidat – zatím necháme default.
...
# -------------------------------
#  R – zatím používá původní R z player.gd
# -------------------------------
func _cast_skill_r() -> void:
    # Prozatím jen použijeme původní implementaci R (tower).
    super._cast_skill_r()
