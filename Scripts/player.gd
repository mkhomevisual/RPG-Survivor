...
func _ready() -> void:
	# Base inicializace (HP, HUD...)
	super._ready()

	# Aby šlo hráče najít přes skupinu
	add_to_group("player")
...
# -------- SKILLY (Q / E / R) – override hooku z CharacterBase --------
...
# -------- SHOOT GLOW IMPLEMENTACE PRO BASE --------
...
# -------- SMĚROVÉ SPRITY (override hooku z CharacterBase) --------
func _update_direction_visual(dir: Vector2) -> void:
	if _sprite == null:
		return

	# Idle – pokud dir == ZERO, buď použijeme tex_idle, nebo necháme poslední texturu
	if dir == Vector2.ZERO:
		if tex_idle:
			_sprite.texture = tex_idle
		return

	# Rozhodneme, jestli převažuje vodorovný nebo svislý pohyb
	if abs(dir.x) >= abs(dir.y):
		# Víc doprava / doleva
		if dir.x > 0.0 and tex_right:
			_sprite.texture = tex_right
		elif dir.x < 0.0 and tex_left:
			_sprite.texture = tex_left
	else:
		# Víc nahoru / dolů
		if dir.y > 0.0 and tex_down:
			_sprite.texture = tex_down
		elif dir.y < 0.0 and tex_up:
			_sprite.texture = tex_up
...
func _die() -> void:
	...
	# Zastavíme hráče, ale nesmažeme ho (kvůli kameře)
	set_process(false)
	set_physics_process(false)
	# pokud máš nějaké kolizní tvary, můžeš je vypnout:
